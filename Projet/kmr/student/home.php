...
<body>
    ...
    if (!empty($_SESSION['CIN'])) {
        $chat_user_id = (string) $_SESSION['CIN'];
        $chat_user_type = 'etudiant';
        $firstName = trim((string) ($_SESSION['nom'] ?? ''));
        $lastName = trim((string) ($_SESSION['prenom'] ?? ''));
        $chat_user_name = trim($firstName . ' ' . $lastName);
        if ($chat_user_name === '') {
            $chat_user_name = 'Etudiant';
        }
